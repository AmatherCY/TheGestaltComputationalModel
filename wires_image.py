...
def find_min_angle_path(start_vertex, single_edge_vertices, points, graph):    
    ...
    # 从起始顶点开始寻找下一个顶点，直到找到另一个在single_edge_vertices中的顶点    
    current_vertex = start_vertex    
    count = 0  
    max_iterations = 4*len(graph.nodes)  # 最大迭代次数，避免无限循环  
...
pd_selected=[]  #筛选出持续时间大于 per_sup 的0PD
for i in range(len(pt_list)):
    if pt_list[i][1]-pt_list[i][0]>=per_sup:
        pd_selected.append(pt_list[i])

threshold = pt_list[1][1]*1.1 #只有一个连通分支
#threshold = pt_list[connect_num][1] #不止一个连通分支
print(threshold)
