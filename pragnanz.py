...
def draw_cycle(cycle, points):
    ...
    plt.savefig('/results/pragnanz.png',dpi=300)

max_dis=50
points = np.array(np.loadtxt('/data/Olympics.txt', dtype=float))
...
def custom_compare(item):  
    return item[1] - item[0] 
dgm_data=ripser.ripser(points)['dgms'][1]
dgm_data=np.array(sorted(dgm_data, key=custom_compare, reverse=True))
...
for i in range(len(pt_list)):
    if pt_list[i][1]-pt_list[i][0]>=per_sup:
        ...
        threshold=max(threshold,pt_list[i][0])
